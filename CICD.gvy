pipeline {
    ...
    stages {
        ...
        stage('Deploy the app to prod Kube master') {
           steps {
              sh 'sudo ansible-playbook --inventory /etc/ansible/hosts train-schedule-kube-deploy.yml --extra-vars "env=prod  build=$BUILD_NUMBER image=elchen8923/train-schedule"'
           }
        }
    }
}
